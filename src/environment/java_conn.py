...
class JavaSimulator:
    def __init__(self, jar_path: str, jvm_options: List[str],
                 ...
                 deterministic_config_dir: str = None):
        ...
        self.SwapPassiveP = "bftsmart.location.management.exploration.instructions.SwapPassive"
        ...
        self.PlacementAvgCalc = "bftsmart/location/placements/PlacementAverageLatencyCalculation"
        ...
        if deterministic_config_dir is not None:
            self.configuration_directories_simulator = [os.path.abspath(base_configuration_directory_simulator
    ...
    def get_number_of_steps(self):
        return self.execution.getWorkload().getNumberOfSteps()
    ...
    def _initialize_objects(self, randomize_starting_positions: bool = False):
        ...
        self.AverageLatencyCalc = jpype.JClass(self.PlacementAvgCalc)
    ...
    def baseline_step(self, action_active: Tuple[int, int], action_passive: Tuple[int, int]) -> FluidityStepResult:
        """
        Execute a baseline step with both active and passive node swaps.

        Args:
            action_active: Tuple of (add_active, remove_active) location indices
            action_passive: Tuple of (add_passive, remove_passive) location indices

        Returns:
            FluidityStepResult containing the simulation state after the step
        """
        action_list = self.JavaArrayList()

        # Handle active swap
        if action_active[0] != action_active[1]:
            all_nodes = self.simulation.getState().getView().getAvailableNodes()
            replicaIDAdd = all_nodes.get(action_active[0])
            replicaIDRemove = all_nodes.get(action_active[1])
            ...
            # Ensure coordinator if needed
            if self.coordinator.equals(replicaIDRemove):
                coordinator = self.EnsureCoordinator(replicaIDAdd)
                action_list.add(coordinator)

        # Handle passive swap
        if action_passive[0] != action_passive[1]:
            all_nodes = self.simulation.getState().getView().getAvailableNodes()
            SwapPassive = jpype.JClass(self.SwapPassiveP)

            passiveIDAdd = all_nodes.get(action_passive[0])
            passiveIDRemove = all_nodes.get(action_passive[1])
            locatedAddNodePassive = self.LocatedNode(passiveIDAdd, passiveIDAdd.getLocation())
            locatedRemoveNodePassive = self.LocatedNode(passiveIDRemove, passiveIDRemove.getLocation())

            swapPassive = SwapPassive(locatedAddNodePassive, locatedRemoveNodePassive)
            action_list.add(swapPassive)

        # Create and execute the step action
        fluidity_action = self.StepAction(self.OptimizationInstructions(action_list))
        step_result = self.execution.executeStep(fluidity_action)
    ...
    def coordinator_swap_step(self, new_coordinator: int) -> FluidityStepResult:
        all_nodes = self.simulation.getState().getView().getAvailableNodes()
        newCoordID = all_nodes.get(new_coordinator)

        ensureCoordinator = self.EnsureCoordinator(newCoordID)
        action_list = self.JavaArrayList()
        action_list.add(ensureCoordinator)
        fluidity_action = self.StepAction(self.OptimizationInstructions(action_list))
    ...
    def get_node_id(self, node_num: int) -> str:
        all_nodes = self.simulation.getState().getView().getAvailableNodes()
        return all_nodes.get(node_num).getLocation().identify()
...
if __name__ == "__main__":
    ...
    '-Djava.security.properties=/home/lukas/flusim/simurun/server0/xmr/config/java.security'],
        ...
        base_configuration_directory_simulator="../../ressources/run_configs/15_locs_100_steps"

    )
    for _ in range(99):
        res = conn.step((-1, -1))
    res = conn.baseline_step((0,0), (1,5))
    res = conn.coordinator_swap_step(1)
    res = conn.coordinator_swap_step(0)
    res = conn.coordinator_swap_step(2)