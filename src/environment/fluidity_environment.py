...
class FluidityEnvironmentConfig:
    ...
    reconfiguration_costs: float = 30
    reduced_state_space: bool = True
    use_reconfig_costs: bool = False
    num_locations: int = None  # Auto-detected from config if None
...
class FluidityEnvironment(Env):
    def __init__(self, config: FluidityEnvironmentConfig):
        ...
        self.reduced_state_space = config.reduced_state_space
        ...
        # Set num_locations from config or auto-detect from location mapping
        if config.num_locations is not None:
            self.num_locations = config.num_locations
        else:
            self.num_locations = len(self.loc_mapping)
            logger.info(f"Auto-detected {self.num_locations} locations from configuration")

        self.action_space = spaces.MultiDiscrete([self.num_locations, self.num_locations])
        ...
        self.graph = nx.Graph()
        self.last_latency = 0
        self.max_steps = self.simulator.get_number_of_steps()
        self.use_reconfig_costs = config.use_reconfig_costs
    ...
    def step(self, action: ActType, passive_action: ActType = None) -> Tuple[ObsType, float, bool, dict]:
        if action[0] == -1 and action[1] == -1:
            # Convert no-op (-1, -1) to valid same-location action using first active location
            first_active_idx = self.inv_loc_mapping[list(self.active_locations)[0]] if self.active_locations else 0
            action = (first_active_idx, first_active_idx)
            logger.debug(f"No action taken (converted to {action})")
        else:
        ...
        if ((self.loc_mapping[action[0]] not in self.available_locations or self.loc_mapping[action[1]] not in self.active_locations)
            ...
            self.last_latency = raw_observation.mean_delay

        else:
            raw_observation  = self.simulator.step(action) if passive_action is None else self.simulator.baseline_step(action, passive_action)
            processed_observation = self._process_raw_observation(raw_observation)
            #reward = self.calculate_reward(processed_observation) + raw_observation.mean_delay #TODO: Check whether we actually need this
            if self.use_reconfig_costs:
        ...
        if self.graph:
            ...
            observation.timestep = raw_observation.step
        else:
        ...
        for loc in raw_observation.distance_latencies.keys():
            ...
            if loc in self.active_locations or loc in self.passive_locations:
                ...
                if self.reduced_state_space:
                    processed_observation[loc] = {x: self.replica_latencies[loc][x] for x in replica_keys}
                else:
...
class TorchGraphObservationWrapper(gym.ObservationWrapper):
    def __init__(self, env, one_hot: bool = False):
        ...
        self.reduced_state_space = env.config.reduced_state_space
    ...
    def observation(self, observation: nx.Graph) -> torch_geometric.data.Data:
        ...
        timesteps = torch.zeros(torch_graph.num_nodes, dtype=torch.long)
        timesteps[torch_graph.label != 0] = observation.timestep
        torch_graph.timestep = timesteps

        if self.one_hot:
            torch_graph.label = torch.nn.functional.one_hot(torch_graph.label)
            torch_graph.timestep = torch.nn.functional.one_hot(torch_graph.timestep, num_classes=self.max_steps).to(torch.float32)
...
if __name__ == "__main__":
    ...
    logger.info(env.active_locations)
    obs, reward, done, _, _ = env.step((1, 4))
    tot_reward.append(reward)
    logger.info(f"Reward: {reward}")
    logger.info(env.active_locations)
    obs, reward, done, _, _ = env.step((6, 2))
    tot_reward.append(reward)
    logger.info(f"Reward: {reward}")
    logger.info(env.active_locations)
