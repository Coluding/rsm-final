...
def dqn(type_embedding_dim: int = 12, hidden_dim: int = 64, action_layer: int = 1, num_locations: int = 8,
    ...
    # Auto-detect num_locations from environment if not explicitly provided
    if num_locations != env.num_locations:
        print(f"Overriding num_locations from {num_locations} to {env.num_locations} (detected from environment)")
    ...
    if action_space == "cross_product":
        # Load action space JSON based on detected number of locations
        action_space_path = f"src/data/action_space_{num_locations}.json"
        print(f"Loading action space from: {action_space_path}")
        ca = CrossProductActionSpace.from_json(action_space_path)
...
def on_policy(type_embedding_dim: int = 12, hidden_dim: int = 64, action_layer: int = 1, num_locations: int = 8,
       ...
       clip_epsilon: float = 0.2, entropy_coeff: float = 0.01, value_coeff: float = 0.5, update_epochs: int = 10,
        ...
        use_client_embeddings=False, on_policy_algorithm="ppo", use_tensorboard: bool = True,
        use_wandb: bool = True, wandb_project: str = "replictated-state-machines-rl", reduced_state_space: bool = True,
            wandb_entity: str = "coluding", wandb_run_name: str = None, run_config: str = "easy/", use_reconfig_costs: bool = True):
    ...
    agent_config = OnPolicyAgentConfig(algorithm=on_policy_algorithm, lr_value_fn=lr_value_fn,
                                       ...
                                       wandb_project=wandb_project, wandb_entity=wandb_entity, wandb_run_name=wandb_run_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_client_embeddings", type=bool, default=False)
    parser.add_argument("--load_path", type=str, default=None)
    parser.add_argument("--run_config", type=str, default="easy/")
    parser.add_argument("--use_reconfig_costs", type=bool, default=False)
    parser.add_argument("--reduced_state_space", type=bool, default=True)
    ...
    if args.algorithm == "dqn":
        ...
        on_policy(type_embedding_dim=args.type_embedding_dim, hidden_dim=args.hidden_dim, action_layer=args.action_layer,
            ...
            use_client_embeddings=args.use_client_embeddings, on_policy_algorithm="PPO", run_config=args.run_config,
            use_reconfig_costs=args.use_reconfig_costs,reduced_state_space=args.reduced_state_space
            )
        ...
        on_policy(type_embedding_dim=args.type_embedding_dim, hidden_dim=args.hidden_dim, action_layer=args.action_layer,
            ...
            reward_scaling=args.reward_scaling, train_every=args.train_every, num_episodes=args.num_episodes,
            stack_states=args.stack_states, action_space=args.action_space, use_timestep_context=args.max_timestep,
            use_client_embeddings=args.use_client_embeddings, on_policy_algorithm="REINFORCE", run_config=args.run_config, reduced_state_space=args.reduced_state_space
            )
