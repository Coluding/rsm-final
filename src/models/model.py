...
class StandardValueModel(BaseValueModel, nn.Module):
    def __init__(self, input_dim: int, embedding_dim: int, hidden_dim: int,
        ...
        self.spatial_encoder = GATEncoder(
            ...
            hidden_dim=hidden_dim, output_dim=hidden_dim, heads=num_heads, max_timestep=max_timestep
        )
        ...
        if max_timestep is not None:
            ...
            self.timestep_embedding = nn.Embedding(max_timestep + 1, hidden_dim)
        ...
        self.non_time_value_head = nn.Sequential(
            nn.Linear(hidden_dim , hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, 1)  # Outputs a single scalar value V(s)
        )
    def forward(self, x, timestep = None, temporal_size: int = 4):
        timestep = None #TODO find where it is called
        all_embeddings = self.spatial_encoder(x)
        B = len(x.batch.unique()) // temporal_size
        ...
        if timestep is not None:
            ...
            value_fn = self.value_head
        else:
            value_fn = self.non_time_value_head

        # Predict state value V(s)
        # Shape: B, 1
        state_value = value_fn(aggregated_temporal_node_embeddings)
