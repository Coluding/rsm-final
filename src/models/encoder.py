...
class GATEncoder(BaseSpatialEncoder, nn.Module):
    def __init__(self, num_layers: int, input_dim, embedding_dim, hidden_dim, output_dim, heads: int = 4,
                 act_fn: str = "gelu", max_timestep: int = 40):
        BaseSpatialEncoder.__init__(self,)
        assert num_layers > 1, "GAT must have at least 2 layers"
        nn.Module.__init__(self)
        self.embedding = nn.Embedding(input_dim, embedding_dim)
        self.request_embedding = nn.Linear(1, embedding_dim)
        # Add +1 to handle 1-indexed timesteps or inclusive ranges
        self.timestep_embedding = nn.Embedding(max_timestep + 2, embedding_dim * 3)

        layers = nn.ModuleList()
        for i in range(num_layers - 1):
            layers.append(gnn.GATConv(embedding_dim * 4 if i == 0 else 0, hidden_dim, heads))
    ...
    def forward(self, data):
        ...
        if x.max() >= self.embedding.num_embeddings:
            raise ValueError(f"Label index out of bounds: max label={x.max().item()}, embedding size={self.embedding.num_embeddings}")
        ...
        try:
            timestep_embedding = self.timestep_embedding((data.timestep))
            ...
            for i, layer in enumerate(self.layers):
                if isinstance(layer, gnn.GATConv):
                    ...
                    x = layer(x)
        except Exception as e: #TODO this is just temporary for debugging
            raise e
