...
class InteractiveActionMapper(SwapActionMapper, nn.Module):
    def __init__(self, num_layer: int, hidden_dim: int, num_locations: int, act_fn: str = "gelu",
        ...
        self.timestep_embedding = nn.Embedding(max_timesteps + 1, hidden_dim) if max_timesteps is not None else None
...
class StandardCrossProductActionMapper(CrossProductSwapActionMapper, nn.Module):
    def __init__(self, num_layer: int, hidden_dim: int, num_locations: int, act_fn: str = "gelu",
        ...
        if max_timestep is not None:
            # Add +1 to handle 1-indexed timesteps or inclusive ranges
            self.timestep_context_embedding = nn.Embedding(max_timestep + 1, hidden_dim)
